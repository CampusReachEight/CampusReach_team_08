...
class PublicProfileViewModelTest {
  ...
  fun loadPublicProfile_cancels_previous_job() = runTest {
    ...
    `when`(repository.getUserProfile("second"))
        .thenReturn(createMockUserProfile(id = "second", name = "Second", lastName = "User"))
  ...
  fun error_is_cleared_when_new_load_starts() = runTest {
    ...
    `when`(repository.getUserProfile("success")).thenReturn(createMockUserProfile(id = "success"))
