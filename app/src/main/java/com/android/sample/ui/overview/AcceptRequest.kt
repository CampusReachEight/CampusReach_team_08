...
import com.android.sample.model.request.RequestRepositoryLocal
        ...
        Column(
            ...
            verticalArrangement = Arrangement.spacedBy(ConstantAcceptRequest.SPACE_BETWEEN_TEXT)) {
              requestState.request?.let { request ->
                ...
                AcceptRequestButton(
                    isAccepted = requestState.accepted,
                    isLoading = requestState.isLoading,
                    onAcceptClick = { acceptRequestViewModel.acceptRequest(requestId) },
                    onCancelClick = { acceptRequestViewModel.cancelAcceptanceToRequest(requestId) })
              }
                  ?: Text(
                      text = "An error occurred. Please, reload window or go back: $errorMessage",
                      fontSize = TEXT_SIZE,
...
@Composable
private fun RequestDetailsContent(request: Request) {
  Text(
  ...
  Text(
      text = "Request type: ${request.requestType.joinToString(", ") { it.displayString() }}",
      fontSize = TEXT_SIZE,
  ...
  Text(
      text = "Start time : ${request.startTime.toDisplayString()}",
      fontSize = TEXT_SIZE,
  ...
  Text(
      ...
      fontSize = TEXT_SIZE,
      modifier = Modifier.testTag(AcceptRequestScreenTestTags.REQUEST_EXPIRATION_TIME))
}

@Composable
private fun ColumnScope.AcceptRequestButton(
    isAccepted: Boolean,
    isLoading: Boolean,
    onAcceptClick: () -> Unit,
    onCancelClick: () -> Unit
  ...
  Button(
      ...
      modifier =
        ...
        if (isLoading) {
          ...
          Text(if (isAccepted) "Cancel" else "Accept")
        }
      }
}
...
@Preview
@Composable
fun AcceptRequestScreenPreview() {
  val fakeRequestData = RequestRepositoryLocal.generateRequestData(1, 1)
  val requestId = fakeRequestData.values.first().keys.first()

  val fakeRepository = RequestRepositoryLocal(requestData = fakeRequestData)
  val viewModel = AcceptRequestViewModel(requestRepository = fakeRepository)

  AcceptRequestScreen(requestId = requestId, acceptRequestViewModel = viewModel, onGoBack = {})
}
