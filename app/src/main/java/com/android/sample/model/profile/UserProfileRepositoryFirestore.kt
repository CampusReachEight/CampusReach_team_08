...
class UserProfileRepositoryFirestore(private val db: FirebaseFirestore) : UserProfileRepository {
  ...
  //
  // The server-side Firestore prefix query approach has a fundamental limitation when combined
  // with client-side filters (section facet, kudos/help range): the "limit mismatch" problem.
  // See UserProfileRepository.kt for detailed rationale.
  //
  // The implementation below is preserved for reference but no longer called.
  // ============================================================================================
  //
  // /**
  //  * Searches public user profiles by first or last name using Firestore prefix range queries.
  //  * Uses the standard [query, query+"\uf8ff") bounds for efficient server-side filtering.
  //  * Performs a second query on lastNameLowercase if needed, deduplicates results and applies
  //  * a client-side contains() filter for substring matching, then enforces the limit.
  //  */
