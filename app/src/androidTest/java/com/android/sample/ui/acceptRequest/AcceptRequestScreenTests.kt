...
class AcceptRequestScreenTests : BaseEmulatorTest() {
  ...
  @Test
  fun volunteersSection_hidesCreatorWhenOnlyCreatorInPeople() {
    // Sign in as default owner
    runTest { signInUser(DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD) }

    // Create a request whose people list incorrectly contains only the creator
    val calendar = Calendar.getInstance()
    calendar.set(2024, Calendar.APRIL, 10, 10, 0, 0)
    calendar.set(Calendar.MILLISECOND, 0)
    val start = Date(calendar.timeInMillis)
    val end = Date(calendar.timeInMillis + 3_600_000)

    val ownerId = currentUserId
    val requestWithCreatorInPeople =
        Request(
            requestId = "request_creator_only",
            title = "Creator only volunteers",
            description = "Should not list creator as volunteer",
            ...
            expirationTime = end,
            people = listOf(ownerId), // faulty data including creator
            tags = emptyList(),
            creatorId = ownerId)

    runTest { repository.addRequest(requestWithCreatorInPeople) }

    composeTestRule.setContent { AcceptRequestScreen("request_creator_only") }

    // Wait for volunteers header (owner only)
    composeTestRule.waitUntil(uiWaitTimeout) {
      composeTestRule
          .onAllNodesWithTag(AcceptRequestScreenTestTags.VOLUNTEERS_SECTION_HEADER)
          .fetchSemanticsNodes()
    ...
    // Since creator is filtered out, list should be empty and show 'No volunteers yet'
    composeTestRule
        .onNodeWithTag(AcceptRequestScreenTestTags.VOLUNTEERS_SECTION_CONTAINER)
        .assertIsDisplayed()
        .assertTextContains("No volunteers yet", substring = true, ignoreCase = true)
  }
}
