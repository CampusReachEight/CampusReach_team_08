...
class MyRequestsScreenTest {
  ...
  class TestViewModel(
    ...
    override val state: StateFlow<MyRequestState>
      get() = super.state
  ...
  fun screen_showsLoadingIndicator_whenLoading() {
    val vm =
        object : MyRequestsViewModel({ emptyList() }) {
          override val state = MutableStateFlow(MyRequestState.loading())
        }
    composeTestRule.setContent { MyRequestsScreen(myRequestsViewModel = vm) }
    ...
    composeTestRule
        .onNode(hasTestTag(MyRequestsTestTags.REQUEST_ADD_BUTTON))
        .assertIsDisplayed()
        .assertIsDisplayed()
  }
  ...
  fun screen_showsListOfRequests_andItemsClickable() {
    val requests = listOf(sampleRequest1, sampleRequest2)
    val vm =
        object : MyRequestsViewModel({ requests }) {
          override val state = MutableStateFlow(MyRequestState(myRequests = requests))
        }
    val nav = mock(NavigationActions::class.java)
    composeTestRule.setContent {
      MyRequestsScreen(navigationActions = nav, myRequestsViewModel = vm)
    }

    composeTestRule
        .onAllNodesWithTag(MyRequestsTestTags.REQUEST_ITEM)
  ...
  fun screen_backButton_navigatesBack_orToProfile() {
    ...
    composeTestRule.setContent { MyRequestsScreen(profileId = "123", navigationActions = nav) }
    ...
    reset(nav)
    composeTestRule.setContent { MyRequestsScreen(profileId = "", navigationActions = nav) }
    composeTestRule.onNodeWithContentDescription("Back to profile").performClick()
    verify(nav).goBack()
  }
  ...
  fun screen_fabButton_navigatesToAddRequest() {
    ...
    composeTestRule.setContent { MyRequestsScreen(navigationActions = nav) }

    composeTestRule
        .onNodeWithTag(MyRequestsTestTags.REQUEST_ADD_BUTTON)
        .assertIsDisplayed()
        .performClick()
